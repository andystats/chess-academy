...
// Tests use level 20 (no blunder injection) so the mocked-engine path is always taken and the
// asserted replies are deterministic; the easy levels deliberately play a random move some of the time.
//
...
describe('useEngineGame', () => {
  it('plays the engine reply after a legal player move', async () => {
    const { result } = renderHook(() => useEngineGame({ fen: START, playerSide: 'white', skillLevel: 20 }));
  ...
  it('asks the engine to move first when the player is Black', async () => {
    const { result } = renderHook(() => useEngineGame({ fen: START, playerSide: 'black', skillLevel: 20 }));
    expect(result.current.status).toBe('engine-thinking');
    expect(requestMove).toHaveBeenCalledTimes(1);
  ...
  it('detects a player win by checkmate', () => {
    const { result } = renderHook(() => useEngineGame({ fen: MATE_IN_ONE, playerSide: 'white', skillLevel: 20 }));
    act(() => {
  ...
  it('detects a draw by stalemate (not a player win)', () => {
    const { result } = renderHook(() => useEngineGame({ fen: STALEMATE_IN_ONE, playerSide: 'white', skillLevel: 20 }));
    act(() => {
...
describe('useEngineGame — guided scenarios', () => {
  ...
  const render = () => renderHook(() => useEngineGame({ fen: PIN, playerSide: 'white', skillLevel: 20, guided }));
