...
import { COLOR_NAME, opposite } from '../lesson/moves.js';
...
// colour from localStorage; the joiner reads them from the ?v / ?host query, validated against the
// variant registry and colour map. Those query values are load-bearing for seat assignment, so a
// mangled link is surfaced as broken instead of silently defaulting (a missing ?host used to make
// both players believe they were black).
export default function OnlinePlayPage() {
  ...
  const config = useMemo(() => {
    ...
    const variant = Object.hasOwn(VARIANTS, v) ? v : null;
    const hostColor = Object.hasOwn(COLOR_NAME, host) ? COLOR_NAME[host] : null;
